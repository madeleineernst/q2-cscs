...
### Unit testing with small mock dataset
...
You can download both a small mass spectral feature table ('small_GNPS_buckettable.qza') as well as an edges file ('small_GNPS_edges.tsv') from this repository:
...
To compute the chemical structural and compositional dissimilarity metric for all pairs of samples for the mock dataset type:
...
The example dataset used here can be accessed at [https://gnps.ucsd.edu/ProteoSAFe/status.jsp?task=5729dd0f7a47475abc879e164c237f56](https://gnps.ucsd.edu/ProteoSAFe/status.jsp?task=5729dd0f7a47475abc879e164c237f56). On the GNPS Job Status page select the “Download Bucket Table” (1.) option. The feature table (a.) as well as the edges file (b.) will be contained within the unzipped folders and files:

![](Example/GNPS_Download.png)

To follow the example below, download the files shown above to your working directory from GNPS using the command line:
